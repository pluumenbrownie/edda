...
summary(ancova_related_lm)
```
While the correlation between crop yield and farm size is significant, the related status does not appear to have an effect.

```{r, fig.height=3, fig.width=6}
qqplot(residuals(ancova_county_lm))
qqline(residuals(ancova_county_lm))
qqplot(residuals(ancova_related_lm))
qqline(residuals(ancova_related_lm))
```
As ANCOVA assumes that the underlying data is normally distributed, we will have to check for normality. 
